...
public class HQ2X {
    public java.awt.image.BufferedImage hq2x(java.awt.image.BufferedImage img) { return img; }

        private static boolean lutsOk = false;
        private static int LUT16to32[];
        private static int RGBtoYUV[];
        private static int YUV1, YUV2;
        ...
        private static int Interp1(int c1, int c2)
        {
          return ((c1*3+c2) >> 2)|0xFF000000;
        }
        
        private static int Interp2(int c1, int c2, int c3)
        {
          return ((c1*2+c2+c3) >> 2)|0xFF000000;
        }
        
        @SuppressWarnings("unused")
        private static int Interp5(int c1, int c2)
        {
          return ((c1+c2) >> 1)|0xFF000000;
        }

        private static int Interp6(int c1, int c2, int c3)
        {
          return (((((c1 & 0x00FF00)*5 + (c2 & 0x00FF00)*2 + (c3 & 0x00FF00) ) & 0x0007F800) +
                         (((c1 & 0xFF00FF)*5 + (c2 & 0xFF00FF)*2 + (c3 & 0xFF00FF) ) & 0x07F807F8)) >> 3)|0xFF000000;
        }

        private static int Interp7(int c1, int c2, int c3)
        {
          return (((((c1 & 0x00FF00)*6 + (c2 & 0x00FF00) + (c3 & 0x00FF00) ) & 0x0007F800) +
                         (((c1 & 0xFF00FF)*6 + (c2 & 0xFF00FF) + (c3 & 0xFF00FF) ) & 0x07F807F8)) >> 3)|0xFF000000;
        }

        private static int Interp9(int c1, int c2, int c3)
        {
          return (((((c1 & 0x00FF00)*2 + ((c2 & 0x00FF00) + (c3 & 0x00FF00))*3 ) & 0x0007F800) +
                         (((c1 & 0xFF00FF)*2 + ((c2 & 0xFF00FF) + (c3 & 0xFF00FF))*3 ) & 0x07F807F8)) >> 3)|0xFF000000;
        }

        private static int Interp10(int c1, int c2, int c3)
        {
          return (((((c1 & 0x00FF00)*14 + (c2 & 0x00FF00) + (c3 & 0x00FF00) ) & 0x000FF000) +
                         (((c1 & 0xFF00FF)*14 + (c2 & 0xFF00FF) + (c3 & 0xFF00FF) ) & 0x0FF00FF0)) >> 4)|0xFF000000;
        }
        
        private static boolean Diff(int w1, int w2)
        {
          YUV1 = RGBtoYUV[w1];
          YUV2 = RGBtoYUV[w2];
        ...
        private static void hq2x_32(int[] in, int[] out, int Xres, int Yres)
        {
          int  i, j, k;
          int  outX = 0;
          int  outY = 0;
          ...
          for (j=0; j<Yres; j++)
          {
            if (j>0)      prevline = -1; else prevline = 0;
            if (j<Yres-1) nextline =  1; else nextline = 0;
        ...
        private static int RGB8888toRGB565(int rgb)
        {
                return ((rgb & 0x00F80000) >> 8) | ((rgb & 0x0000FC00) >> 5) | ((rgb & 0x000000F8) >> 3);  
        }
       

        private static BufferedImage addAlpha (BufferedImage image)
        {
            ...
            for (int y = 0; y < image.getHeight(); y++) {
            	...
            	for (int x = 0; x < width; x++) {
            		...
            		int alpha = (imgData[x] & 0xFF000000) << 8; // shift red into alpha bits
            		color |= alpha;
            		imgData[x] = color;
            	}
            	// replace the data
        ...
        private static BufferedImage removeAlpha (BufferedImage image)
        {
            ...
            for (int y = 0; y < image.getHeight(); y++) {
            	...
            	for (int x = 0; x < width; x++) {
            		int color = imgData[x] & 0x00FFFFFF; // mask away any alpha present
            		int alpha = (imgData[x] & 0x00FF0000) << 8; // shift red into alpha bits
            		color |= alpha;
            		imgData[x] = color;
            	}
            	// replace the data
        ...
        public BufferedImage HQ2X(BufferedImage in)
        {
            ...
            int x = w*2;
            int y = h*2;
            int[] inArr  = new int[w*h];
            int[] outArr = new int[w*h*4];
		...
		/*
		public static Bitmap Parse(Bitmap in) 
		{
			int w = in.getWidth();
			int h = in.getHeight();
			...
			int[] inArr  = new int[w*h];
			int[] outArr = new int[w*h*4];
			in.getPixels(inArr, 0, w, 0, 0, w, h);
			if (!lutsOk){
				LUT16to32 = new int[65536];
				RGBtoYUV  = new int[65536];
				InitLUTs();
			...
			return Bitmap.createBitmap(outArr, w*2, h*2, Bitmap.Config.ARGB_8888);
		}
		
		public static Bitmap Parse(Resources res, int inId) 
		{
			Bitmap in = null;
			
			try{
				in = BitmapFactory.decodeResource(res, inId);
			}catch(Exception e){
