			...
			for(int n=numStrings;n>0;n--)
			{
				...
				if(dt.x!=-1 || dt.y!=-1)
				{
					...
					if(x==-1)
					{
						...
						x = (int)(LWJGLUtils.SCREEN_SIZE_X/2-((GLUtils.getStringWidth(dt.text)+3)/2));
					}
					...
					GLUtils.drawOutlinedString(dt.text, x, y, dt.color);

				}
				else
				{
					messagesCounter++;
					GLUtils.drawOutlinedString(dt.text, 8, LWJGLUtils.SCREEN_SIZE_Y-(12*messagesCounter), dt.color);
				}
			}
