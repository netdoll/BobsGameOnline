...
{//=========================================================================================================================
	...
	{//=========================================================================================================================
		...
		if(scrolling==false&&maxWidth<caption.texture.getWidth())
		{
			scrolling=true;
			scrollX = maxWidth;
		...
		if(scrolling==true)
		{
			...
			if(scrollX<0-caption.texture.getWidth())scrollX=maxWidth;
	...
	{//=========================================================================================================================
		...
		if(layer==0)
		{
			...
			if(hasProgressBar==true)
			{
				...
				float bgtx1 = bgw/(float)bg.getWidth();
				float bgty1 = bgh/(float)bg.getHeight();
				GL.drawTexture(bg,0,bgtx1,0,bgty1,screenX,screenX+maxWidth,0,25,1.0f,1);*/


				Texture fg = NotificationManager.loadingBarTexture;
				float fgImageWidth = fg.getWidth();
				float fgTextureWidth = fg.getWidth();
				float fgImageHeight = fg.getHeight();
				float fgTextureHeight = fg.getHeight();
			...
			{
				float tx0 = 0.0f;
				float tx1 = (float)caption.texture.getWidth()/(float)caption.texture.getWidth();
				float ty0 = 0.0f;
				float ty1 = (float)caption.texture.getHeight()/(float)caption.texture.getHeight();

				float x0 = (caption.screenX);
				float x1 = (caption.screenX+caption.texture.getWidth());
				float y0 = (caption.screenY);
				float y1 = (caption.screenY+caption.texture.getHeight());
				...
				if(scrolling==true)
				{
					...
					if(scrollX<0)
					{
						...
						x0 = caption.screenX;
						tx0 = (0-scrollX)/(float)caption.texture.getWidth();

						//if the rest will fit
						if(scrollX+caption.texture.getWidth()<maxWidth)
						{
							tx1 = (float)caption.texture.getWidth()/(float)caption.texture.getWidth();
							x1 = caption.screenX + scrollX+caption.texture.getWidth();
						}
						else
						{
							//clip x1 and tx1

							tx1 = ((0-scrollX)+maxWidth)/(float)caption.texture.getWidth();
					...
					else
					{
						...
						if(scrollX + caption.texture.getWidth() > maxWidth)
						{
							...
							tx1 = (maxWidth-scrollX)/(float)caption.texture.getWidth();

						}
						else
						{
							//draw the whole thing
							x1 = x0 + caption.texture.getWidth();
							tx1 = (float)caption.texture.getWidth()/(float)caption.texture.getWidth();
				...
				else
				{
					...
					x0 = (caption.screenX);
					x1 = (caption.screenX+caption.texture.getWidth());
					y0 = (caption.screenY);
					y1 = (caption.screenY+caption.texture.getHeight());
				}
