...
//				if(textColor==BobColor.YELLOW)this.textAAColor=BobColor.DARKYELLOW;
	...
	{//=========================================================================================================================
		...
		float tx0 = 0.0f;
		float tx1 = (float)width/(float)texWidth;
		float ty0 = 0.0f;
		float ty1 = (float)height/(float)texHeight;

		float x0 = (screenX);
		float x1 = ((screenX)+(width*scale));
		float y0 = (screenY);
		float y1 = ((screenY)+(height*scale));


		int filter=GLUtils.FILTER_NEAREST;
		if(scale==0.5f || scale==1.0f || scale==2.0f)filter = GLUtils.FILTER_NEAREST;
		else filter = GLUtils.FILTER_LINEAR;
